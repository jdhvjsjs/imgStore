...
> 让这个适配器类去实现接口，对接口里面的所有的方法做==空重写==。
> 让==子类继承这个适配器类==，想要用到哪个方法，就重写哪个方法。
...
​	内部类被private修饰，外界无法直接获取内部类的对象，只能通过==3.3节中的方式二获取内部类的对象==
...
**实际上，如果我们希望==定义一个只要使用一次的类，就可考虑使用匿名内部类==。匿名内部类的本质作用**
